...
public class CampsUI extends JPanel /* implements ActionListener */ {
	...
	/*
	 * It should be noted that the following class was completed using a
	 * tutorial online as a template. The tutorial can be found at the following
	 * URL:
	 * 
	...
	public CampsUI() {
		setBorder(new TitledBorder(new EtchedBorder(), "Camp Details"));
	...
	private void setFieldData(Camps c) {
		...
		try {
			...
			endDatePicker.getJFormattedTextField().setText(endDatePicker.getModel().getYear() + "-"
					+ endDatePicker.getModel().getMonth() + "-" + endDatePicker.getModel().getDay());
		}
	...
	private boolean isEmptyFieldData() {
		...
		return (nameField.getText().trim().isEmpty() || priceWeekly.getText().trim().isEmpty()
				|| priceDaily.getText().trim().isEmpty() || priceHalfDay.getText().trim().isEmpty()
				|| datePicker.getModel().getValue() == null || endDatePicker.getModel().getValue() == null);
	}
		...
		public void actionPerformed(ActionEvent e) {
			...
			case "Save":
				...
				if (bean.create(c) != null)
					JOptionPane.showMessageDialog(null, "New camp created successfully.");
